...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('id,status,actual_minutes,hours_confirmed,dobro_hours_entered,admin_comment,evidence_comment,evidence_url,created_at,vacancy:bt_vacancies(id,title,event_date,start_time,place,format,confirmation_text)')
